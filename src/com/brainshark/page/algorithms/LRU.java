...
public class LRU {
	...
	public void algorithms(int block) {
		...
		System.out.println("LRU算法" + block + "内存");
		if (block == 3) {
		...
		for (int i = 0; i < pagingStorage.page.length; i++) {
			...
			if (paramExist(memory, pagingStorage.page[i])) {
				...
				if (emptyIndex == -1) {
					...
					int minIndex = getMinIndex(memory, pagingStorage.page, i);
					memory[minIndex] = pagingStorage.page[i];
					memoryReplace(memory, minIndex, block);
				} else {
	...
	public int getMinIndex(int[] memory, int[] page, int i) {
		int maxBlockIndex = 1000;
		int maxPlaceIndex = 1000;
		for (int blockIndex = 0; blockIndex < memory.length; blockIndex++) {
			for (int placeIndex = i - 1; placeIndex < page.length; placeIndex--) {
				if (page[placeIndex] == memory[blockIndex]) {
					if (maxPlaceIndex > placeIndex) {
						maxPlaceIndex = placeIndex;
